...
build-documentation-standalone: ## Build documentation (standalone)
	...
	docker run \
		...
		&& $(MAKE) -j 4 && python3.11 -m pip install -r /app/docs/docs-requirements.txt && ostk-install-python && cd /app/docs/ && python3.11 clean_xml.py && sphinx-build -c rst/ rst/ _build"
...
clean: ## Clean
	...
	rm -rf "$(CURDIR)/docs/xml"
	rm -rf "$(CURDIR)/docs/_build"
	rm -rf "$(CURDIR)/docs/rst/api"
	rm -rf "$(CURDIR)/lib"
