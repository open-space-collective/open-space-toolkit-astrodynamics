...
class Sequence
{
   ...
   private:
    ...
    /// @brief Build a coast segment from a segment. All of the new segment attributes remain the same with
    /// the exception of the dynamics, which will not include the thruster dynamics.
    ///
    /// @param aSegment A segment
    /// @param aSuffix Suffix to append to the segment name
    /// @return A coast segment
    Segment buildCoastSegment_(const Segment& aSegment, const String& aSuffix) const;
    ...
    /// @brief Build an array of coast dynamics from a segment. All of the new dynamics will remain the same with
    /// the exception of the thruster dynamics, which will be excluded.
    ///
    /// @param aSegment A segment
    /// @return An array of coast dynamics
    Array<Shared<Dynamics>> buildCoastDynamics_(const Segment& aSegment) const;
