...
Size ObservationFilter::Analysis::getInlierCount() const
{
    ...
    for (Eigen::Index i = 0; i < inlierMask.size(); ++i)
    {
        if (inlierMask(i) != 0)
        {
            ++count;
...
Array<State> ObservationFilter::apply(const Array<State>& anObservationStateArray) const
{
    return this->filter(anObservationStateArray)->getFilteredObservations(anObservationStateArray);
}
