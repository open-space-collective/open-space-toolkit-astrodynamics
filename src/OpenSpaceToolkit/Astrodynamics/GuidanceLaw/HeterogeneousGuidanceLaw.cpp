...
Shared<GuidanceLaw> HeterogeneousGuidanceLaw::createInstanceForManeuverExtraction() const
{
    ...
    for (Size index = 0; index < guidanceLaws_.getSize(); ++index)
    {
        heterogeneousGuidanceLaw->addGuidanceLaw(
            guidanceLaws_[index]->createInstanceForManeuverExtraction(), intervals_[index]
        );
    }
