...
) const
{
    ...
    StateBuilder estimationStateBuilder(anInitialGuessState);
    const Shared<const Frame>& estimationStateFrame = anInitialGuessState.accessFrame();
    StateBuilder referenceStateBuilder(aReferenceStateArray[0]);
    ...
    for (Size iteration = 0; iteration < maxIterationCount_; ++iteration)
    {
        ...
        for (Size i = 0; i < referenceStateCount; ++i)
        {
            ...
            const MatrixXd H = HFull.block(i * referenceStateDimension, 0, referenceStateDimension, stateDimension);
...
VectorXd LeastSquaresSolver::expandSigmas(
    const std::unordered_map<CoordinateSubset, Real>& aSigmas, const StateBuilder& aStateBuilder
)
{
    ...
    for (Size i = 0; i < stateSubsets.getSize(); ++i)
    {
        ...
        if (sigmaIt->second <= 0.0)
        {
            throw ostk::core::error::RuntimeError("Sigma value must be greater than 0.");
        }

        const Real scaledSigma = sigmaIt->second / std::sqrt(subsetSize);
        sigmasCoordinates.segment(currentIndex, subsetSize).setConstant(scaledSigma);
