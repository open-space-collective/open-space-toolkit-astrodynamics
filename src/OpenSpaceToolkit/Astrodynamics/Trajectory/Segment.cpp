...
) const
{
    ...
    const auto solveSingleManeuver = [&](const Shared<Thruster>& thrusterDynamics,
                                         const Instant& continueUntilInstant = Instant::Undefined()
                                     ) -> std::pair<Segment::Solution, Array<flightManeuver>>
    {
        const State& lastState = segmentStates.accessLast();
        const Duration solveDuration = continueUntilInstant.isDefined()
                                         ? continueUntilInstant - lastState.accessInstant()
                                         : maximumInstant - lastState.accessInstant();
        const bool stopAtFirstThrustCutOff = continueUntilInstant.isDefined() ? false : true;
        const Segment::Solution maneuverSolution =
            solveLOFCompliantManeuverSubsegment_(lastState, solveDuration, thrusterDynamics, stopAtFirstThrustCutOff);
        ...
        switch (maneuverConstraints_.maximumDurationStrategy)
        {
            ...
            case MaximumManeuverDurationViolationStrategy::TruncateEnd:
            {
                ...
                const auto [maneuverSolution, _] =
                    solveSingleManeuver(slicedThruster, candidateManeuverInterval.getEnd());
