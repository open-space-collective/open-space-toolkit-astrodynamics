...
State Tabulated::calculateStateAt(const Instant& anInstant) const
{
    ...
    if ((stateRange.first != nullptr) && (stateRange.second != nullptr))
    {
        ...
        const Vector3d x_ECI = previousState.accessPosition().accessCoordinates() +
                               ratio * (nextState.accessPosition().accessCoordinates() -
                                        previousState.accessPosition().accessCoordinates());
        const Vector3d v_ECI = previousState.accessVelocity().accessCoordinates() +
                               ratio * (nextState.accessVelocity().accessCoordinates() -
                                        previousState.accessVelocity().accessCoordinates());

        const Position position = {x_ECI, previousState.accessPosition().getUnit(),
                                   previousState.accessPosition().accessFrame()};
        const Velocity velocity = {v_ECI, previousState.accessVelocity().getUnit(),
