...
CartesianPosition::CartesianPosition(const String& aName, const Size& aSize) : CoordinatesSubset(aName, aSize)
{
    if (aSize > 3) {
...
CartesianPosition::CartesianPosition(const Size& aSize) : CartesianPosition(CartesianPosition::DEFAULT_NAME, aSize)
{

}

VectorXd CartesianPosition::inFrame(const Instant& anInstant, const VectorXd& allCoordinates, const Shared<const Frame>& fromFrame, const Shared<const CoordinatesBroker>& aCoordinatesBroker, const Shared<const Frame>& toFrame) const {
    ...
    Vector3d toFrameCoordinates = Position::Meters(
        {positionCoordinates(0), positionCoordinates(1), positionCoordinates(2)},
        fromFrame
    ).inFrame(toFrame, anInstant).getCoordinates(); 
