                    ...
                    namespace kepler
                    {
                        ...
                        BMLOE BMLOE::Cartesian(const COE::CartesianState &aCartesianState, const Derived &aGravitationalParameter)
                        {
                            ...
                            const auto getAeq = [](const Vector6d &aCOEVector) -> Vector6d
                            {
                                Vector6d aeq;
                                aeq[0] = aCOEVector[0];
                                aeq[1] = aCOEVector[1] * sin(aCOEVector[4] + aCOEVector[3]);
                                aeq[2] = aCOEVector[1] * cos(aCOEVector[4] + aCOEVector[3]);
                                aeq[3] = sin(aCOEVector[2] / 2.0) * sin(aCOEVector[3]);
                                aeq[4] = sin(aCOEVector[2] / 2.0) * cos(aCOEVector[3]);
                                aeq[5] = aCOEVector[2] + aCOEVector[3] + aCOEVector[4];
                            ...
                            Vector6d coeVector2 = BMLOE::COE(coeVector, aGravitationalParameter);
                            ...
                            while (emag > 1e-8)
                            {
                                ...
                                coeVector2 = BMLOE::COE(blmean2, aGravitationalParameter);
                                cartesian2 = COE::FromVector(kep2, COE::AnomalyType::Mean).getCartesian();
                                ...
                                if (emag_old > emag)
                                {
                                    ...
                                    seq2 = getAqe(coeVector2);
                                    aeqmean = aeqmean2;
                                    aeqmean2 = aeqmean + (aeq - aeq2);
                                }
                                else
                                {
                                    throw ostk::core::error::RuntimeError("Brouwer mean long algorithm cannot converge.");
                                }
                                if (iterator > 75)
                                {
                                    throw ostk::core::error::RuntimeError("Brouwer mean long algorithm cannot converge.");
                        ...
                        Vector6d BMLOE::COE(const Vector6d &aCOEVector, const Derived &aGravitationalParameter)
                        {
                            ...
                            const Real ae = 1.0;
                            const Real smadp = aCOEVector[0] / equatorialRadius;
                            const Real eccdp = aCOEVector[1];
                            const Real incdp = aCOEVector[2];
                            const Real raandp = aCOEVector[3];
                            const Real aopdp = aCOEVector[4];
                            const Real meanAnom = aCOEVector[5];
                            ...
                            const Real sinMADP = sin(meanAnom);
                            const Real cosMADP = sin(meanAnom);
                            const Real sinraandp = sin(raandp);
                            const Real cosraandp = cos(raandp);

                            // True anomaly (double primed)
                            const Real tadp = COE::TrueAnomalyFromEccentricAnomaly(COE::EccentricAnomalyFromMeanAnomaly(Angle::Radians(meanAnom), eccdp, 1e-12), eccdp);
                            ;

                            const Real rp = smadp * (1.0 - eccdp * eccdp) / (1.0 + eccdp * cos(tadp));
                            const Real adr = smadp / rp;
                            const Real sinta = sin(tadp);
                            const Real costa = cos(tadp);
                            const Real cs2gta = cos(2.0 * aopdp + 2.0 * tadp);
                            ...
                            const Real sinI2 = sin(incdp / 2.0);
                            const Real cosI2 = cos(incdp / 2.0);
                            const Real tanI2 = Tan(incdp / 2.0);
                            const Real a26 = 16.0 * a16 + 40.0 * a17 + 3.0;
                            const Real a27 = a22 * (1.0 / 8.0) * (11.0 + 200.0 * a17 + 80.0 * a16);

                            const Real b1 = cn * (a1 - a2) - ((a11 - 400.0 * a15 - 40.0 * a14 - 11.0 * a13) * (1.0 / 16.0) + (11.0 + 200.0 * a17 + 80.0 * a16) * a22 * (1.0 / 8.0)) * gmp2 + ((-80.0 * a15 - 8.0 * a14 - 3.0 * a13 + a11) * (5.0 / 24.0) + (5.0 / 12.0) * a26 * a22) * g4dg2;
                            const Real b2 = a6 * a19 * (2.0 + cn - eccdp2) + (5.0 / 64.0) * a5 * a19 * cn2 - (15.0 / 32.0) * a4 * a18 * cn * cn2 + ((5.0 / 64.0) * a5 + a6) * a21 * tanI2 + (9.0 * eccdp2 + 26.0) * (5.0 / 64.0) * a4 * a18 + (15.0 / 32.0) * a3 * a21 * a26 * sinI * (1.0 - theta);
                            const Real b3 = ((80.0 * a17 + 5.0 + 32.0 * a16) * a22 * sinI * (theta - 1.0) * (35.0 / 576.0) * g5dg2 * eccdp) - ((a22 * tanI2 + (2.0 * eccdp2 + 3.0 * (1.0 - cn2 * cn)) * sinI) * (35.0 / 1152.0) * a8p);
                            const Real b4 = cn * eccdp * (a1 - a2);
                            const Real b5 = ((9.0 * eccdp2 + 4.0) * a10 * a4 * (5.0 / 64.0) + a7) * cn;
                            const Real b6 = (35.0 / 384.0) * a8 * cn2 * cn * sinI;
                            const Real b7 = ((cn2 * a18) / (1.0 - 5.0 * theta2)) * ((1.0 / 8.0) * gmp2 * (1.0 - 15.0 * theta2) + (1.0 - 7.0 * theta2) * g4dg2 * (-(5.0 / 12.0)));
                            const Real b8 = (5.0 / 64.0) * (a3 * cn2 * (1.0 - 9.0 * theta2 - (24.0 * theta4 / (1.0 - 5.0 * theta2)))) + a6 * cn2;
                            ...
                            const Real b12 = -((80.0 * a17 + 32.0 * a16 + 5.0) * (a22 * eccdp * sinI * sinI * (35.0 / 576.0) * g5dg2) + (a8 * a21 * (35.0 / 1152.0)));

                            // Semi-Major Axis
                            const Real sma = smadp * (1.0 + gm2 * ((3.0 * theta2 - 1.0) * (eccdp2 / (cn2 * cn2 * cn2)) * (cn + (1.0 / (1.0 + cn))) + ((3.0 * theta2 - 1.0) / (cn2 * cn2 * cn2)) * (eccdp * costa) * (3.0 + 3.0 * eccdp * costa + eccdp2 * costa2) + 3.0 * (1.0 - theta2) * adr3 * cs2gta));
                            const Real sn2gta = sin(2.0 * aopdp + 2.0 * tadp);
                            const Real snf2gd = sin(2.0 * aopdp + tadp);
                            const Real csf2gd = cos(2.0 * aopdp + tadp);
                            const Real sn2gd = sin(2.0 * aopdp);
                            const Real cs2gd = cos(2.0 * aopdp);
                            const Real sin3gd = sin(3.0 * aopdp);
                            const Real cs3gd = cos(3.0 * aopdp);
                            const Real sn3fgd = sin(3.0 * tadp + 2.0 * aopdp);
                            const Real cs3fgd = cos(3.0 * tadp + 2.0 * aopdp);
                            const Real sinGD = sin(aopdp);
                            const Real cosGD = cos(aopdp);
                            ...
                            // L + G + H
                            const Real blgh = blghp + ((1.0 / (cn + 1.0)) * (1.0 / 4.0) * eccdp * gmp2 * cn2 * (3.0 * (1.0 - theta2) * (sn3fgd * ((1.0 / 3.0) + adr2 * cn2 + adr) + snf2gd * (1.0 - (adr2 * cn2 + adr))) + 2.0 * sinta * (3.0 * theta2 - 1.0) * (adr2 * cn2 + adr + 1.0))) + gmp2 * (3.0 / 2.0) * ((-2.0 * theta - 1.0 + 5.0 * theta2) * (eccdp * sinta + tadp - meanAnom)) + (3.0 + 2.0 * theta - 5.0 * theta2) * (gmp2 * (1.0 / 4.0) * (eccdp * sn3fgd + 3.0 * (sn2gta + eccdp * snf2gd)));
                            ...
                            const Real dlte = dlt1e + ((1.0 / 2.0) * cn2 * ((3.0 * (1.0 / (cn2 * cn2 * cn2)) * gm2 * (1.0 - theta2) * cs2gta * (3.0 * eccdp * costa2 + 3.0 * costa + eccdp2 * costa * costa2 + eccdp)) - (gmp2 * (1.0 - theta2) * (3.0 * csf2gd + cs3fgd)) + (3.0 * theta2 - 1.0) * gm2 * (1.0 / (cn2 * cn2 * cn2)) * (eccdp * cn + (eccdp / (1.0 + cn)) + 3.0 * eccdp * costa2 + 3.0 * costa + eccdp2 * costa * costa2)));
                            ...
                            // Inclination
                            const Real inc = 2 * std::asin(sqrI);
