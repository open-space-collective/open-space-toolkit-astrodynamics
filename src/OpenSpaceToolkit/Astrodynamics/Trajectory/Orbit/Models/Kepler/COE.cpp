...
Length COE::getPeriapsisRadius() const
{
    ...
    return this->semi_major_axis_ * (1.0 - this->eccentricity_);
}
...
Length COE::getApoapsisRadius() const
{
    ...
    return this->semi_major_axis_ * this->eccentricity_;
}
...
Vector6d COE::asVector(const AnomalyType& anAnomalyType) const
{
    return {
        ...
        raan_.inRadians(),
        aop_.inRadians(),
        COE::ParseAnomaly(trueAnomaly_, anAnomalyType),
    };
}
...
COE COE::FromVector(const Vector6d &aCOEVector, const AnomalyType &anAnomalyType)
{
    return {
        ...
        COE::ParseAnomaly(Angle::Radians(aCOEVector[5]), anAnomalyType),
    };
}
...
{
    return TrueAnomalyFromEccentricAnomaly(EccentricAnomalyFromMeanAnomaly(aMeanAnomly, anEccentricity, aTolerance), anEccentricity);
}
