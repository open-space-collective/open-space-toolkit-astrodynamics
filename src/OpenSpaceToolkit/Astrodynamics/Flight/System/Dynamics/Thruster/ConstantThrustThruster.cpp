...
void ConstantThrustThruster::applyContribution(const NumericalSolver::StateVector& x, NumericalSolver::StateVector& dxdt, const Instant& anInstant) const
{
    ...
    const Position position_GCRF = {x_GCRF, Position::Unit::Meter, Frame::GCRF()};
    const Velocity velocity_GCRF = {v_GCRF, Velocity::Unit::MeterPerSecond, Frame::GCRF()};
    ...
    Shared<const LOFProvider> providerSPtr = (frameSPtr->accessProvider());

    // providerSPtr->getTransformAt(anInstant, x_GCRF, v_GCRF).getOrientation();
    ...
    const Vector3d acceleration_GCRF = R_GCRF_LOF * acceleration_VNC;
