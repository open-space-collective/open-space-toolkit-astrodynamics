...
@pytest.fixture
def sample_gps_telemetry_columns() -> list[str]:
    return [
        "_time",
...
@pytest.fixture
def sample_gps_telemetry(sample_gps_telemetry_columns: list[str]) -> pd.DataFrame:
    coordinates: list = [
        [
    ...
    return pd.DataFrame(
        data=coordinates, columns=sample_gps_telemetry_columns
    ).set_index("_time")
