...
class TestConjunctiveCondition:
    def test_constructor(self, event_conditions: list[EventCondition]):
        assert (
            Conjunctive(event_conditions) is not None
        )

    def test_is_satisfied(self, conjunction_condition: Conjunctive):
        # x0 crossing root (true), x1 negative (true)
        assert conjunction_condition.is_satisfied([1.0, 0.0], 0.0, [-1.0, 3.0], 0.0) == True
    
        # x0 crossing root (true), x1 positive (false)
        assert conjunction_condition.is_satisfied([1.0, 1.0], 0.0, [-1.0, 3.0], 0.0) == False
        
        # x0 not crossing root (false), x1 negative (true)
        assert conjunction_condition.is_satisfied([-0.5, 1.0], 0.0, [-1.0, 3.0], 0.0) == False
    
        # x0 not crossing root (false), x1 positive (false)
        assert conjunction_condition.is_satisfied([-0.5, 0.0], 0.0, [-1.0, 3.0], 0.0) == False