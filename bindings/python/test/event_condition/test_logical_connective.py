...
class TestLogicalConnective:
    def test_constructor(self, event_conditions: list[EventCondition]):
        assert (
            LogicalConnective("Logical Conjunctive", event_conditions) is not None
        )

    def test_get_event_conditions(self, logical_connective: LogicalConnective, event_conditions: list[EventCondition]):
