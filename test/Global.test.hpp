...
#define EXPECT_INSTANTS_ALMOST_EQUAL(i1, i2, tolerance)                                                  \
    ...
    {                                                                                                    \
        EXPECT_TRUE((i1).isNear((i2), (tolerance)))                                                      \
            << "Expected " << (i1).toString() << " to be near " << (i2).toString() << " with tolerance " \
            << (tolerance).toString() << " but was " << ((i1) - (i2)).toString();                        \
    } while (0)
...
#define EXPECT_DURATIONS_ALMOST_EQUAL(d1, d2, tolerance)                                                 \
    ...
    {                                                                                                    \
        EXPECT_TRUE((d1).isNear((d2), (tolerance)))                                                      \
            << "Expected " << (d1).toString() << " to be near " << (d2).toString() << " with tolerance " \
            << (tolerance).toString() << " but was " << ((d1) - (d2)).toString();                        \
    } while (0)
