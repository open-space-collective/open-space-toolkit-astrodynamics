...
class OpenSpaceToolkit_Astrodynamics_Trajectory_Segment_Solve_QLawHangingScenariosWhenReachingThreshold
...
INSTANTIATE_TEST_SUITE_P(
    QLawHangingScenariosWhenReachingThreshold,
    OpenSpaceToolkit_Astrodynamics_Trajectory_Segment_Solve_QLawHangingScenariosWhenReachingThreshold,
    ::testing::Values(
        // Scenario 1: SMA crosses its threshold and starts slowing down the propagation
        ...
        // Scenario 2: SMA crosses its threshold and starts slowing down the propagation
        QLawHangingScenarioParams {
            "SMA_WithEff",
            Duration::Hours(13.0),
            1.0,                   // smaWeight
        ...
        // Scenario 3: Eccentricity crosses its threshold and starts slowing down the propagation
        QLawHangingScenarioParams {
            "ECC_NoEff",
            Duration::Hours(10.0),
            0.0,                   // smaWeight
...
TEST_P(OpenSpaceToolkit_Astrodynamics_Trajectory_Segment_Solve_QLawHangingScenariosWhenReachingThreshold, HangingScenariosWhenReachingThreshold)
{
    const auto params = GetParam();
...
class OpenSpaceToolkit_Astrodynamics_Trajectory_Segment_Solve_QLawHangingScenariosWhenReachingTarget
    : public ::testing::TestWithParam<QLawHangingScenarioParams>
{
   protected:
    void SetUp() override
    {
        ...
        satelliteSystem_ = SatelliteSystem(
            ...
            satelliteGeometry,
            Matrix3d::Identity(),
            1.9075,                       // cross-sectional area
            2.2,                          // drag coefficient
            PropulsionSystem(0.0161, 1140.26)  // thrust (N), specific impulse (s)
        );

        // Initial state (2026-01-18T12:00:00 UTC)
        initialInstant_ = Instant::DateTime(DateTime(2026, 1, 19, 10, 0, 0), Scale::UTC);
        coordinateBrokerSPtr_ = std::make_shared<CoordinateBroker>(CoordinateBroker({
            CartesianPosition::Default(),
            CartesianVelocity::Default(),
            CoordinateSubset::Mass(),
        ...
        VectorXd initialCoordinates(9);
        initialCoordinates << 649189.5860162771, -6857629.054702876, 182381.35043757217,  // position (m)
            -979.7357665756809, -303.39558122275594, -7538.521186774207,                  // velocity (m/s)
            198.2,                                                                        // mass (kg)
            1.9075,                                                                       // surface area (m^2)
            2.2;                                                                          // drag coefficient
    ...
    // Tolerances (same for all scenarios)
    const double smaTolerance_ = 200.0;             // meters
    const double eccTolerance_ = 1.0;
    const double incTolerance_ = 0.0;               // radians
    const double raanTolerance_ = 0.0;              // radians
    const double aopTolerance_ = 0.0;               // radians
...
INSTANTIATE_TEST_SUITE_P(
    QLawHangingScenariosWhenReachingTarget,
    OpenSpaceToolkit_Astrodynamics_Trajectory_Segment_Solve_QLawHangingScenariosWhenReachingTarget,
    ::testing::Values(
        // Scenario 1: SMA is out, and ECC is inside its box (which is huge), it's still targeted and it slows down once ECC reaches its target
        QLawHangingScenarioParams {
            "SMA_And_Ecc",
            Duration::Hours(12.0),
            1.0,                   // smaWeight
            ...
            0.0,                   // raanWeight
            0.0,                   // aopWeight
            0.7,                   // absoluteEffectivityThreshold
            0.7,                   // relativeEffectivityThreshold
        }
    ),
    [](const ::testing::TestParamInfo<QLawHangingScenarioParams>& paramInfo)
    {
...
TEST_P(OpenSpaceToolkit_Astrodynamics_Trajectory_Segment_Solve_QLawHangingScenariosWhenReachingTarget, HangingScenariosWhenReachingTarget)
{
    const auto params = GetParam();
    ...
    const COE initialBLMAsCOE = initialBLM_.toCOE();
    const COE targetCOE = {
        Length::Meters(6901136.3),           // target semi-major axis
        0.0003,                              // target eccentricity
        initialBLM_.getInclination(),        // keep initial inclination
        initialBLM_.getRaan(),               // keep initial RAAN
        Angle::Degrees(90.0),                // target AoP
        initialBLMAsCOE.getTrueAnomaly(),    // true anomaly (not targeted, for constructor consistency)
