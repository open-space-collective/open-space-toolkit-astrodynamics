...
/// @file           OpenSpaceToolkit/Astrodynamics/Trajectory/Orbit/Utilities.test.hpp
...
void testOrbit(const String& aScenarioIdentifier, const File& aReferenceDataFile, const Orbit& anOrbit,
               const Real& aPositionTolerance_GCRF, const Real& aVelocityTolerance_GCRF,
               const Real& aPositionTolerance_ITRF, const Real& aVelocityTolerance_ITRF)
{
    ...
    for (const auto& referenceRow : referenceData)
    {
        ...
        ASSERT_GT(aPositionTolerance_GCRF, (position_GCRF.accessCoordinates() - referencePosition_GCRF).norm())
            << String::Format("[{}] {}: {} ≉ {} Δ {} > {} [m]", aScenarioIdentifier, instant.toString(),
                              referencePosition_GCRF.toString(), position_GCRF.accessCoordinates().toString(),
        ...
        ASSERT_GT(aVelocityTolerance_GCRF, (velocity_GCRF.accessCoordinates() - referenceVelocity_GCRF).norm())
            << String::Format("[{}] {}: {} ≉ {} Δ {} > {} [m/s]", aScenarioIdentifier, instant.toString(),
                              referenceVelocity_GCRF.toString(), velocity_GCRF.accessCoordinates().toString(),
        ...
        ASSERT_GT(aPositionTolerance_ITRF, (position_ITRF.accessCoordinates() - referencePosition_ITRF).norm())
            << String::Format("[{}] {}: {} ≉ {} Δ {} > {} [m]", aScenarioIdentifier, instant.toString(),
                              referencePosition_ITRF.toString(), position_ITRF.accessCoordinates().toString(),
        ...
        ASSERT_GT(aVelocityTolerance_ITRF, (velocity_ITRF.accessCoordinates() - referenceVelocity_ITRF).norm())
            << String::Format("[{}] {}: {} ≉ {} Δ {} > {} [m/s]", aScenarioIdentifier, instant.toString(),
                              referenceVelocity_ITRF.toString(), velocity_ITRF.accessCoordinates().toString(),
                              (velocity_ITRF.accessCoordinates() - referenceVelocity_ITRF).norm(),
                              aVelocityTolerance_ITRF);

        ASSERT_EQ(referenceRevolutionNumber.floor(), anOrbit.getRevolutionNumberAt(instant))
            << String::Format("[{}] {}: {} ≠ {}", aScenarioIdentifier, instant.toString(),
                              referenceRevolutionNumber.floor(), anOrbit.getRevolutionNumberAt(instant));
    }
}
