...
TEST(OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagated, Constructor)
{
    ...
    // Constructor without putting into orbit object
    {
        ...
        const State state_0 = {instant_0, Position::Meters({0.0, 0.0, 0.0}, gcrfSPtr),
        ...
        const State state_1 = {instant_1, Position::Meters({0.0, 0.0, 0.0}, gcrfSPtr),
                               Velocity::MetersPerSecond({1.0, 0.0, 0.0}, gcrfSPtr)};
        ...
        const Composite satelliteGeometry(
            Cuboid({0.0, 0.0, 0.0}, {Vector3d{1.0, 0.0, 0.0}, Vector3d{0.0, 1.0, 0.0}, Vector3d{0.0, 0.0, 1.0}},
                   {1.0, 2.0, 3.0}));
        const SatelliteSystem satelliteSystem = {Mass(100.0, Mass::Unit::Kilogram), satelliteGeometry,
        ...
        NumericalSolver numericalSolver = {NumericalSolver::LogType::NoLog,
...
TEST(OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagated, EqualToOperator)
{
...

    {
        ...
        const State state = {startInstant, Position::Meters({0.0, 0.0, 0.0}, gcrfSPtr),
                             Velocity::MetersPerSecond({1.0, 0.0, 0.0}, gcrfSPtr)};
...
TEST(OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagated, CalculateStateAt)
{
    ...
    // Setup instants
    const Array<Instant> instantArray = {Instant::DateTime(DateTime(2018, 1, 2, 0, 0, 0), Scale::UTC),
...
TEST(OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagated, CalculateStatesAt)
{
    ...
    // Test interlocking of new states into cachedStateArray in terms of instants
    {
        ...
        const State state_1 = {Instant::DateTime(DateTime(2018, 1, 1, 22, 0, 0), Scale::UTC),
                               Position::Meters({7000000.0, 0.0, 0.0}, gcrfSPtr),
                               Velocity::MetersPerSecond({0.0, 5335.865450622126, 5335.865450622126}, gcrfSPtr)};
        cachedStateArray.add(state_1);
        const State state_2 = {Instant::DateTime(DateTime(2018, 1, 2, 0, 0, 0), Scale::UTC),
                               Position::Meters({7000000.0, 0.0, 0.0}, gcrfSPtr),
                               Velocity::MetersPerSecond({0.0, 5335.865450622126, 5335.865450622126}, gcrfSPtr)};
        cachedStateArray.add(state_2);
        const State state_3 = {Instant::DateTime(DateTime(2018, 1, 2, 2, 0, 0), Scale::UTC),
                               Position::Meters({7000000.0, 0.0, 0.0}, gcrfSPtr),
                               Velocity::MetersPerSecond({0.0, 5335.865450622126, 5335.865450622126}, gcrfSPtr)};
    ...
    // Test cachedStateArray with one cachedState at the beginning of reference states
    {
        ...
        // Since none of the reference states used are duplicates, the cached state array after propagation should be
        // the same length as all the reference states
    ...
    // Test cachedStateArray with two cachedStates, one at the beginning and one at the end of reference states
    {
        ...
        const State state_1 = {
            ...
            Position::Meters({referencePositionArray[cachedStateReferenceIndex_1]}, gcrfSPtr),
            Velocity::MetersPerSecond({referenceVelocityArray[cachedStateReferenceIndex_1]}, gcrfSPtr)};
        cachedStateArray.add(state_1);
        size_t cachedStateReferenceIndex_2 = 60;
        const State state_2 = {
            instantArray[cachedStateReferenceIndex_2],
            Position::Meters({referencePositionArray[cachedStateReferenceIndex_2]}, gcrfSPtr),
            Velocity::MetersPerSecond({referenceVelocityArray[cachedStateReferenceIndex_2]}, gcrfSPtr)};
        cachedStateArray.add(state_2);
    ...
    // Test cachedStateArray with half of reference states already in cachedStateArray
    {
        ...
        for (size_t i = 0; i < instantArray.getSize(); i++)
        {
            if (i % 2 == 0)
            {
                const State state = {instantArray[i], Position::Meters({referencePositionArray[i]}, gcrfSPtr),
                                     Velocity::MetersPerSecond({referenceVelocityArray[i]}, gcrfSPtr)};
...
TEST(OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagated, CalculateRevolutionNumberAt)
{
    ...
    // is using
    {
        ...
        const State state = {startInstant, Position::Meters({7000000.0, 0.0, 0.0}, gcrfSPtr),
                             Velocity::MetersPerSecond({0.0, 5335.865450622126, 5335.865450622126}, gcrfSPtr)};
...
TEST(OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagated, CachedStateArray)
{
    ...
    // Test cachedStateArray sorting during construction and accessing (not during propagation)
    {
        ...
        // Current state and instant setup
        Array<State> stateArray = Array<State>::Empty();
        const State state_1 = {Instant::DateTime(DateTime(2018, 1, 1, 0, 0, 0), Scale::UTC),
                               Position::Meters({7000000.0, 0.0, 0.0}, gcrfSPtr),
                               Velocity::MetersPerSecond({0.0, 5335.865450622126, 5335.865450622126}, gcrfSPtr)};
        stateArray.add(state_1);
        const State state_2 = {Instant::DateTime(DateTime(2018, 1, 2, 0, 0, 0), Scale::UTC),
        ...
        const State state_3 = {Instant::DateTime(DateTime(2018, 1, 3, 0, 0, 0), Scale::UTC),
                               Position::Meters({7000000.0, 0.0, 0.0}, gcrfSPtr),
                               Velocity::MetersPerSecond({0.0, 5335.865450622126, 5335.865450622126}, gcrfSPtr)};
...

    {
        ...
        // Satellite dynamics setup
        const State state_4 = {Instant::DateTime(DateTime(2018, 1, 4, 0, 0, 0), Scale::UTC),
                               Position::Meters({7000000.0, 0.0, 0.0}, gcrfSPtr),
                               Velocity::MetersPerSecond({0.0, 5335.865450622126, 5335.865450622126}, gcrfSPtr)};
        SatelliteDynamics satelliteDynamics = {defaultEnvironment, satelliteSystem};
...

    {
        ...
        const State state_1 = {Instant::DateTime(DateTime(2018, 1, 1, 0, 0, 0), Scale::UTC),
                               ...
                               Velocity::MetersPerSecond({0.0, 5335.865450622126, 5335.865450622126}, gcrfSPtr)};
        stateArray.add(state_1);
        const State state_2 = {Instant::DateTime(DateTime(2018, 1, 1, 0, 0, 0), Scale::UTC),
                               Position::Meters({8000000.0, 0.0, 0.0}, gcrfSPtr),
                               Velocity::MetersPerSecond({0.0, 5335.865450622126, 5335.865450622126}, gcrfSPtr)};
        stateArray.add(state_2);
...
TEST(OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagated, PropAccuracy_TwoBody)
{
    ...
    // Two body vs GMAT
    {
        ...
        const State state = {startInstant, Position::Meters({referencePositionArray_GCRF[0]}, gcrfSPtr),
...
TEST(DISABLED_OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagated,
     PropAccuracy_ExtendedDuration)  // These tests take a long time and are for local testing only (not to be run on
                                     // every push to git)
{
    using ostk::core::ctnr::Array;
    using ostk::core::ctnr::Table;
...
TEST(DISABLED_OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagated, PropAccuracy_SpeedTest)
{
    ...
    // Benchmark the performance of a particular configuration vs TwoBody
    {
        ...
        for (size_t i = 0; i < timingTestSampleSize; i++)
        {
            ...
            const Array<Shared<Object>> objects_tested = {std::make_shared<Earth>(Earth::EGM2008(70, 70)),
                                                          std::make_shared<Moon>(Moon::Spherical()),
                                                          std::make_shared<Sun>(Sun::Spherical())};
            ...
            const Orbit orbit_default = {propagatedModel_default,
                                         customEnvironment_default.accessCelestialObjectWithName("Earth")};
            const Orbit orbit_tested = {propagatedModel_tested,
                                        customEnvironment_tested.accessCelestialObjectWithName("Earth")};
        ...
        const double timeAverage_default =
            std::accumulate(timeArray_default.begin(), timeArray_default.end(), 0.0) / timeArray_default.getSize();
        std::cout << "timeAverage_default: " << timeAverage_default << std::endl;
        const double timeAverage_tested =
            std::accumulate(timeArray_tested.begin(), timeArray_tested.end(), 0.0) / timeArray_tested.getSize();
