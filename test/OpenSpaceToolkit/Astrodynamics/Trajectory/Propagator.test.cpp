...
TEST_F(OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagator, Constructor)
{
    ...
    {
        EXPECT_NO_THROW(Propagator(dynamics_, numericalSolver_));
    }
}
...
TEST_F(OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagator, OrekitSpehrical)
{
    ...
    const State postBurnState = propagator.calculateStateAt(state, state.getInstant() + Duration::Minutes(60.0));

    VectorXd expectedCoordinates(6);
    expectedCoordinates << -5172890.413848375000,-4716058.194055955000,0000000.000000000000,05083.946625786208,-05576.415230079790,00000.000000000000;
...
TEST_F(OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagator, OrekitExponential)
{
    ...
    VectorXd expectedCoordinates(6);
    expectedCoordinates << -5172889.585695211000,-4716058.453899897000,0000000.000008572841,05083.947538266920,-05576.414764013522,-00000.000000010687;
...
TEST_F(OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagator, ConstantThrustManeuverWithDrag)
{
    ...
    expectedCoordinates << -5320761.901959980000,-4668633.969927717000,0000000.000006977032,04920.638509410778,-05657.638329841361,-00000.000000008794,99.755268109;
...
TEST_F(OpenSpaceToolkit_Astrodynamics_Trajectory_Orbit_Models_Propagator, ConstantThrustManeuver)
{
    ...
    // Setup Propagator model and orbit
    dynamics_.add(std::make_shared<GravitationalDynamics>(GravitationalDynamics(Earth::Spherical())));
    dynamics_.add(std::make_shared<ThrusterDynamics>(ThrusterDynamics({1.0, 1500.0}, Direction({1.0, 0.0, 0.0}, gcrfSPtr_))));  // TBI: Set VNC frame here
    const Propagator propagator = {dynamics_, numericalSolver_};
